...
def fetch_sp400_symbols():
    """Fetch S&P Mid-Cap 400 constituent symbols"""
    print("Fetching S&P Mid-Cap 400 constituents from Wikipedia...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✓ Fetched {len(symbols)} S&P 400 symbols")
            return symbols
...
def main():
    print("=" * 70)
    print("S&P MID-CAP 400 (IJH/MDY) SCRAPER")
    print("=" * 70)
    ...
    filename = f"SP400_MidCap_IJH_{datetime.now().strftime('%Y%m%d')}.csv"
    df.to_csv(filename, index=False)
    
    print(f"\n{'='*70}")
    print(f"✓ Found {len(filtered_stocks)} mid-cap stocks")
